...
import { Star, FlaskConical, Droplets, Microscope, ShoppingCart, Check, Phone } from 'lucide-react';
...
const TestCard = ({ test }) => {
    ...
    const title = test?.testName || test?.test || test?.name || 'Health Test';
        ...
        >
            ...
            <div className="px-5 pb-4">
                <span className="inline-flex items-center gap-1.5 text-[11px] font-black text-emerald-700 uppercase tracking-wider bg-emerald-50 border border-emerald-200 rounded-full px-3 py-1.5">
                    ...
                    {test?.parametersCount || test?.parameters?.length || 1} Parameters
                </span>
            </div>
            ...
            <div className="flex gap-6 px-5 pb-4 border-t border-gray-100 pt-3">
                <div>
                    ...
                    <p className="text-[11px] font-black text-gray-700 uppercase">{test?.tat || '24 Hrs'}</p>
                </div>
                <div>
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Method</p>
                    <p className="text-[11px] font-black text-gray-700 uppercase">{test?.method || 'EIA/CLIA'}</p>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between px-5 pb-5 mt-auto">
                <div className="flex items-center gap-1">
                    ...
                    <span className="text-[13px] font-black text-gray-700">{test?.rating || '4.5'}</span>
                </div>
