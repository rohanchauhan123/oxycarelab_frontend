...
import { ArrowRight, Sparkles, FlaskConical, Droplets, Microscope, Star, ShoppingCart, Check, Phone } from 'lucide-react';
...
const fallbackItems = [
    ...
    { id: 'f9', name: 'Iron Studies', category: 'Pathology', price: 399, originalPrice: 650, sampleType: 'Blood', tat: '24 Hrs', method: 'Automated', rating: 4.6, parameters: [1, 2, 3] },
    { id: 'f10', name: 'Vitamin B12', category: 'Pathology', price: 499, originalPrice: 799, sampleType: 'Blood', tat: '24 Hrs', method: 'CLIA', rating: 4.8, parameters: [1] },
];
...
const SliderCard = ({ pkg }) => {
    ...
    return (
        <div
            ...
            className="w-[230px] shrink-0 bg-white rounded-2xl border border-gray-200 flex flex-col cursor-pointer overflow-hidden group hover:border-emerald-300 hover:-translate-y-1 transition-all duration-300"
                ...
                <div className="flex gap-1.5">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1 px-3 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-wide transition-all ${
                            isInCart ? 'bg-emerald-100 text-emerald-700' : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm active:scale-95'
                        }`}
                    >
...
const PopularPackages = () => {
    ...
    const displayItems = React.useMemo(() => {
        const pkgs = (packages || []).filter(p => p?.status !== 'Deleted' && p?.status !== 'Inactive');
        return pkgs.length > 0 ? pkgs : fallbackItems;
    }, [packages]);

    // Triple the items — ensures seamless loop even at different screen widths
    const items = [...displayItems, ...displayItems, ...displayItems];

    // Card width = 230px, gap = 20px → each item = 250px
    // Original set width = 250 * N items
    // We animate translateX from 0 to -(250 * displayItems.length)px
    const itemWidth = 250; // 230px card + 20px gap
    const singleSetWidth = itemWidth * displayItems.length;

    return (
        <section className="py-20 bg-gray-50/70 relative overflow-hidden">
            {/* Inject animation styles directly — no DOM style tag conflicts */}
            <style>{`
                ...
                .marquee-infinite {
                    animation: marqueeInfinite ${Math.max(displayItems.length * 3, 20)}s linear infinite;
                    will-change: transform;
                }
                .marquee-outer-wrap:hover .marquee-infinite {
            ...
            <div className="relative z-10">
                ...
                <div className="relative marquee-outer-wrap">
                    {/* Edge fades */}
                    ...
                    <div className="overflow-hidden">
                        {/* Track: tripled items, fixed pixel animation — guaranteed infinite */}
                        <div
                            className="marquee-infinite flex"
